...
fetch('http://www.omdbapi.com/?i=tt3896198&apikey=c89fb4d4')
    .then(res => res.json())
...
//             const url = `http://www.omdbapi.com/?i=${id}&plot=full&apikey=c89fb4d4`;
//             const res = await fetch(url);
//             const data = await res.json();
//             console.log(data);
...
async function favoritesMovieLoader() {
    ...
    for (let i = 0; i < keys.length; i++) {
        const id = localStorage.getItem(keys[i]);
        if (id) {
            try {
                const url = `http://www.omdbapi.com/?i=${id}&plot=full&apikey=c89fb4d4`;
                const res = await fetch(url);
                ...
                // Ensure the data contains the expected movie details
                if (!data || data.Response === "False") {
                    console.error(`Error in data for ID ${id}: ${data.Error}`);
                    continue;
                }

                const img = data.Poster ? data.Poster : data.Title;
    ...
    // Appending the HTML to the movie-display class in the favorites page
    document.querySelector('.fav-container').innerHTML = output;
}
